...
프로메테우스 컨피그를 에디트 하고 테스트 하는 기능은 `Edit`, `Save & Test`를 통해 동작합니다. 해당 단계에서는 컨피그의 YAML 문법 오류와 컨피그의 기본적인 오류가 있는지를 체크합니다.  

정상적으로 테스트가 종료된 컨피그는 아래와 같이 테스트가 통과되면 임시로 저장되게 되고, 실제로 프로메테우스 클러스터에 적용이 되지 않은 상태입니다.

![img](../static/img/nc-config-test.png)

테스트를 실패하는 경우를 살펴보겠습니다. 예를 들어 `global.scrape_timeout`의 경우 Second 단위(s)로 입력할 경우 60초를 초과하는 값을 입력하게 되면 비정상으로 간주되어 테스트가 실패하게 됩니다. 실패의 경우에는 관련 로그를 화면에 출력해서 보여주게 됩니다.  
